...
interface SprintPendenciasSessionProps {
  preset?: Preset;
  tasks?: unknown;
  onClose: () => void;
  onComplete?: () => void;
...
export default function SprintPendenciasSession({ preset, tasks, onClose, onComplete }: SprintPendenciasSessionProps) {
  ...
  const [step,             setStep]             = useState<string>(saved?.step             ?? "setup");
  const [projetoNome,      setProjetoNome]      = useState<string>(saved?.projetoNome      ?? "");
  const [pendencias,       setPendencias]       = useState<Pendencia[]>(saved?.pendencias  ?? []);
  const [sprintCount,      setSprintCount]      = useState<number>(saved?.sprintCount      ?? 0);
  const [currentIdx,       setCurrentIdx]       = useState<number>(saved?.currentIdx       ?? 0);
  const [novaInput,        setNovaInput]        = useState<string>("");
  ...
  const removePendencia = (id: number): void => {
    setPendencias((prev) => prev.filter((p) => p.id !== id));
  };

  const marcarFeita = (): void => {
    setPendencias((prev) =>
