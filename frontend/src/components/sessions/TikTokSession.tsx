...
export default function TikTokSession({
  preset,
  tasks,
  onCompleteTask,
  onToggleChecklist,
  ...
  onClose,
  onComplete,
}: TikTokSessionProps): JSX.Element {
  ...
  const [cycle,        setCycle]        = useState<number>(saved?.cycle        ?? 1);
  const [taskInCycle,  setTaskInCycle]  = useState<number>(saved?.taskInCycle  ?? 0);
  ...
  const [savingSubtask, setSavingSubtask] = useState<boolean>(false);
  const [wasRestored,   setWasRestored]   = useState<boolean>(!!saved);

  const [baseVideos,    setBaseVideos]    = useState<number>(saved?.baseVideos    ?? 5);
  const [videoDuration, setVideoDuration] = useState<number>(saved?.videoDuration ?? 60);
  const [maxCycles,     setMaxCycles]     = useState<number>(saved?.maxCycles     ?? 0);

  const [videosCount,       setVideosCount]       = useState<number>(saved?.videosCount       ?? 0);
  const [streak,            setStreak]            = useState<number>(saved?.streak            ?? 0);
  const [cycleHistory,      setCycleHistory]      = useState<CycleHistoryEntry[]>(saved?.cycleHistory      ?? []);
  const [currentCycleTasks, setCurrentCycleTasks] = useState<string[]>(saved?.currentCycleTasks ?? []);
  ...
  const resetSubtask = (): void => { setShowSubtask(false); setSubtaskText(""); };
  ...
  const startWatching = (): void => { setVideosCount(0); setTaskInCycle(0); setStep("watching"); };

  const nextVideoCount = (): void => setVideosCount((v) => Math.min(v + 1, numVideos));

  const advanceAfterTaskComplete = (removedTaskId: string, taskName: string | undefined): void => {
    resetSubtask();
    setCompleted((c) => c + 1);
  ...
  const advanceAfterSubtaskComplete = (taskName: string | undefined): void => {
    resetSubtask();
    setCompleted((c) => c + 1);
    ...
    <div className={styles.root}>
      ...
      <div className={styles.body}>
        ...
        {step === "working" && selectedTask && (() => {
          ...
          const hasChecklist = (live.checklist?.length ?? 0) > 0;
          return (
            <>
              <div className={styles.cycleBadge}>Ciclo {cycle} — {taskInCycle + 1} de {numTasks}</div>
              <div className={styles.taskDisplay}>
                ...
                {hasChecklist ? (
                  <SubtaskFlow
                    ...
                    onToggle={(itemId: string) => handleSubtaskToggle(live.id, itemId)}
              ...
              {showSubtask && (
                <form onSubmit={handleAddSubtask} className={styles.actions}>
                  <input
                    ...
                    value={subtaskText}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSubtaskText(e.target.value)}
