...
interface CafeRitualSessionProps {
  tasks: TaskItem[];
...
type Step = "ritual" | "music_hunt" | "register_song" | "select_task" | "working" | "cycle_done" | "summary";

export default function CafeRitualSession({ tasks, onCompleteTask, onToggleChecklist, onAddChecklist, onClose, onComplete }: CafeRitualSessionProps) {
  const { saved, persist, clearSaved } = useSessionPersist("caferitual");

  const [step,         setStep]         = useState<Step>(saved?.step         ?? "ritual");
  const [completed,    setCompleted]    = useState<number>(saved?.completed    ?? 0);
  const [cycles,       setCycles]       = useState<number>(saved?.cycles       ?? 0);
  const [songName,     setSongName]     = useState<string>(saved?.songName     ?? "");
  const [songInput,    setSongInput]    = useState<string>("");
  const [doneIds,      setDoneIds]      = useState<Set<string>>(() => new Set(saved?.doneIds ?? []));
  const [selectedTask, setSelectedTask] = useState<TaskItem | null>(() => {
  ...
  const renderChecklist = (live: TaskItem): JSX.Element | false =>
    (live.checklist?.length ?? 0) > 0 && (
      <SubtaskFlow
      ...
      <div className={styles.body}>
        ...
        {step === "select_task" && (
          <>
            ...
            <TaskSelector
              tasks={available}
              onSelect={(t: TaskItem) => { setSelectedTask(t); setStep("working"); }}
