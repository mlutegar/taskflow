...
export default function PomodoroSession({ preset, tasks, onCompleteTask, onToggleChecklist, onAddChecklist, onClose, onComplete }: PomodoroSessionProps) {
  const {
  ...
  const [step,     setStep]     = useState<string>(saved?.step     ?? "pick_duration");
  const [duration, setDuration] = useState<number>(saved?.duration ?? preset?.duration ?? 25);
  const [rounds,   setRounds]   = useState<number>(saved?.rounds   ?? 0);
  ...
  return (
    ...
    >
        ...
        {step === "select_task" && (
          <TaskSelector
            tasks={available}
            onSelect={(t: Task) => { setSelectedTask(t); setStep("timing"); }}
        ...
        {step === "timing" && (
          <>
            ...
            <CountdownTimer
              ...
              onComplete={() => { setRounds((r: number) => r + 1); setStep("post_timer"); }}
              onCancel={() => { setRounds((r: number) => r + 1); setStep("post_timer"); }}
            />
          </>
        )}
