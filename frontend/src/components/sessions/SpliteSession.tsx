...
function randomDiaryDate(): string {
...
export default function SpliteSession({ preset, tasks, onCompleteTask, onToggleChecklist, onAddChecklist, onClose, onComplete }: SpliteSessionProps) {
  const {
  ...
  const [step,          setStep]          = useState<string>(saved?.step          ?? presetInit?.step       ?? "select_activity");
  const [activity,      setActivity]      = useState<string | null>(saved?.activity      ?? presetInit?.activity   ?? null);
  const [cycle,         setCycle]         = useState<number>(saved?.cycle         ?? 1);
  const [taskInCycle,   setTaskInCycle]   = useState<number>(saved?.taskInCycle   ?? 0);
  const [isDiaryMode,   setIsDiaryMode]   = useState<boolean>(saved?.isDiaryMode   ?? presetInit?.isDiaryMode ?? false);
  const [nextDiaryStep, setNextDiaryStep] = useState<string>(saved?.nextDiaryStep ?? "reading_analysis");
  const [diaryDate,     setDiaryDate]     = useState<string | null>(saved?.diaryDate     ?? presetInit?.diaryDate  ?? null);
  const [activities,    setActivities]    = useState<string[]>(() => getActivities());
  const [newActivity,   setNewActivity]   = useState<string>("");
  const [pinned,        setPinned]        = useState<string[]>(() => getPinned());

  // Recorde pessoal (usado quando preset.trackRecord === true)
  const recordKey: string | null = preset?.recordLabel ? `taskflow.record.${preset.recordLabel}` : null;
  const [record, setRecord] = useState<number>(() => recordKey ? (parseInt(localStorage.getItem(recordKey) ?? "0") || 0) : 0);
  ...
  const handleRemoveActivity = (a: string): void => setActivities(removeActivity(a));
  const handleTogglePin = (a: string): void => setPinned(togglePin(a));

  const selectActivity = (a: string): void => {
    setActivity(a);
      ...
      <div className={styles.body}>
          ...
          <>
            ...
            <div className={styles.actionsRow} style={{ marginTop: 10 }}>
              <input
                ...
                onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") handleAddActivity(); }}
              />
              <button
                ...
                <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <input
                      ...
                      value={currentCount}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentCount(e.target.value)}
                      style={{ width: 160 }}
                    />
        ...
        {step === "cycle_done" && (
          <>
            ...
            <button className={`${styles.btn} ${styles.btnPrimary}`} onClick={() => { setCycle((c: number) => c + 1); setStep("doing_activity"); }}>Próximo ciclo</button>
