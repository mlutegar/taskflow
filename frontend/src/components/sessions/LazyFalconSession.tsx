...
interface LazyFalconSessionProps {
  tasks: Task[];
  onCompleteTask: (id: string) => Promise<void>;
  onToggleChecklist: (...args: unknown[]) => unknown;
  onAddChecklist: (...args: unknown[]) => unknown;
  onClose: () => void;
  onComplete?: () => void;
}

const LS_KEY = "taskflow_lazyfal_saved";
function loadSaved(): SavedTask[] { try { return JSON.parse(localStorage.getItem(LS_KEY) as string) || []; } catch { return []; } }
function persistSaved(d: SavedTask[]): void { localStorage.setItem(LS_KEY, JSON.stringify(d)); }

export default function LazyFalconSession({ tasks, onCompleteTask, onToggleChecklist, onAddChecklist, onClose, onComplete }: LazyFalconSessionProps) {
  const {
  ...
  const [step,        setStep]        = useState<string>(sessState?.step        ?? "select_activity");
  const [activity,    setActivity]    = useState<string | null>(sessState?.activity    ?? null);
  const [cycle,       setCycle]       = useState<number>(sessState?.cycle       ?? 1);
  const [taskInCycle, setTaskInCycle] = useState<number>(sessState?.taskInCycle ?? 0);
  const [activities,  setActivities]  = useState<string[]>(() => getActivities());
  const [newActivity, setNewActivity] = useState<string>("");

  const [saved,     setSaved]     = useState<SavedTask[]>(() => loadSaved());
  ...
  const doSave = (note: string | null): void => {
      ...
      <div className={styles.body}>
        ...
        {step === "select_task" && (
          <>
            ...
            <TaskSelector
              tasks={available}
              onSelect={(t: Task) => { setSelectedTask(t); setSubStep(null); setStep("task_action"); }}
