...
const PHASES: { key: string; label: string; emoji: string; duration: number; color: string; instruction: string }[] = [
...
export default function BreathingSession({ preset, onClose, onComplete }: BreathingSessionProps) {
  const [phase, setPhase] = useState<string>("idle"); // idle | breathing | done
  ...
  const runTimer = (pIdx: number, cycleNum: number): void => {
  ...
  return (
    <div className={styles.root}>
      <SessionHeader
        ...
        onClose={() => { clearInterval(intervalRef.current ?? undefined); onClose(); }}
      />
