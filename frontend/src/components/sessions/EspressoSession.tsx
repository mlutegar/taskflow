...
interface Task {
  ...
  description?: string;
  completed?: boolean;
  checklist?: { id: string; [key: string]: unknown }[];
  [key: string]: unknown;
}

interface EspressoSessionProps {
  tasks: Task[];
...
type Step = "coffee_check" | "select_task" | "ready_sprint" | "timing" | "post_sprint" | "summary";

export default function EspressoSession({ tasks, onCompleteTask, onToggleChecklist, onAddChecklist, onClose, onComplete }: EspressoSessionProps) {
  const { saved, persist, clearSaved } = useSessionPersist("espresso");

  const [step,        setStep]        = useState<Step>(saved?.step      ?? "coffee_check");
  const [completed,   setCompleted]   = useState<number>(saved?.completed ?? 0);
  const [coffees,     setCoffees]     = useState<number>(saved?.coffees   ?? 0);
  const [sprints,     setSprints]     = useState<number>(saved?.sprints   ?? 0);
  ...
  return (
    ...
    >
        ...
        {step === "select_task" && (
          <TaskSelector
            tasks={available}
            onSelect={(t: Task) => { setSelectedTask(t); setStep("ready_sprint"); }}
