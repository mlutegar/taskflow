...
interface TabHopSessionProps {
  onClose: () => void;
  onComplete?: () => void;
}

export default function TabHopSession({ onClose, onComplete }: TabHopSessionProps) {
  const { saved, persist, clearSaved } = useSessionPersist("tabhop");

  const [step,        setStep]        = useState<string>(saved?.step        ?? "context");
  const [tabs,        setTabs]        = useState<Tab[]>(saved?.tabs        ?? []);
  const [currentIdx,  setCurrentIdx]  = useState<number>(saved?.currentIdx  ?? 0);
  const [cycle,       setCycle]       = useState<number>(saved?.cycle       ?? 0);
  ...
  const finishAddTab = (name: string): void => {
    const trimmed = name.trim();
    if (!trimmed) return;
