...
function SongManager({ songs, onAdd, onRemove }: SongManagerProps): JSX.Element {
  const [input, setInput] = useState<string>("");
  const submit = (): void => { onAdd(input); setInput(""); };
  return (
...
interface SingSessionProps {
  preset: { variant?: string } | null | undefined;
  tasks: Task[];
...
export default function SingSession({ preset, tasks, onCompleteTask, onToggleChecklist, onAddChecklist, onClose, onComplete }: SingSessionProps): JSX.Element {
  ...
  const [step,      setStep]      = useState<string>(saved?.step  ?? (variant === "ten" ? "hunt" : "sing"));
  const [queue,     setQueue]     = useState<string[]>(saved?.queue ?? []);
  const [songInput, setSongInput] = useState<string>("");
  const [songs,     setSongs]     = useState<string[]>(() => getSongs());
  const [suggestion, setSuggestion] = useState<string>(() => randomSong());
  ...
  const reroll = (): void => setSuggestion(randomSong(suggestion));
  const handleAddSong = (name: string): void => { const list = addSong(name); setSongs(list); };
  const handleRemoveSong = (name: string): void => { const list = removeSong(name); setSongs(list); if (suggestion === name) setSuggestion(randomSong(name)); };
  ...
  const registerQueueSong = (name: string): void => {
    setQueue((q: string[]) => [...q, name]);
    setSongInput("");
    ...
    <div className={styles.root}>
      ...
      <div className={styles.body}>
        ...
        {step === "working" && selectedTask && (() => {
          const live = tasks.find((t: Task) => t.id === selectedTask.id) || selectedTask;
