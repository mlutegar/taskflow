...
interface RoutineCardProps {
  ...
  onCompleteForDate: (id: string | number, date: string) => Promise<void>;
  ...
  onAddProgress: (id: string | number, value: number) => Promise<void>;
  onAddChecklist: (id: string | number, description: string) => Promise<void>;
  onToggleChecklist: (routineId: string | number, itemId: string | number) => void;
  onDeleteChecklist: (routineId: string | number, itemId: string | number) => void;
}
...
function RoutineCard({
  ...
  const [pastDate, setPastDate] = useState<string>(yesterday);
  ...
  const handleSaveEdit = async (): Promise<void> => {
    ...
    try {
      await onUpdate(routine.id, {
        ...
        description: (editData.description as string).trim() || null,
        target_value: editData.target_value !== "" ? Number(editData.target_value) : null,
        unit: (editData.unit as string).trim() || null,
      });
      setEditing(false);
    } finally {
      setSaving(false);
  ...
  const handleAddChecklist = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
        ...
        <div className={styles.editForm}>
          ...
          <div className={styles.editRow}>
            <input
              type="number"
              className={styles.editInput}
              value={editData.target_value as string}
    ...
    <div className={`${styles.card} ${routine.is_completed_today ? styles.done : ""}`}>
      <div className={styles.main}>
        ...
        >
          ...
          {!hasTarget && (routine.checklist_count ?? 0) > 0 && (
