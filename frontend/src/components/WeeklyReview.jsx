...
export default function WeeklyReview({ onClose }) {
  ...
  const { weekSessions, topMode, bestMode, totalTasks, totalFocusMin } = useMemo(() => {
    ...
    const weekSessions = allHistory.filter((e) => toIso(e.date) >= cutoff);
    const totalTasks = weekSessions.reduce((s, e) => s + e.tasks.length, 0);

    const logs = getUsageLogs().filter((l) => l.date >= cutoff);
    const totalFocusMin = logs.reduce((s, l) => s + (l.focusedMinutes || 0), 0);

    const stats = usageStats(7);
    const topModeEntry = stats[0];
    const topMode = topModeEntry ? MODES.find((m) => m.id === topModeEntry.modeId) : null;
    ...
    const bestMode = bestEntry ? MODES.find((m) => m.id === bestEntry.id) : null;
          ...
          <div className={styles.section}>
            ...
            {topMode && (
              <div className={styles.insight}>
                <span className={styles.insightEmoji}>{topMode.emoji}</span>
                <div>
                  <strong>{topMode.name}</strong> foi seu modo mais usado essa semana.
                </div>
              </div>
            )}
            ...
            {bestMode && (
              <div className={`${styles.insight} ${styles.insightSuccess}`}>
                <span className={styles.insightEmoji}>{bestMode.emoji}</span>
                <div>
                  <strong>{bestMode.name}</strong> teve a melhor taxa de sucesso da semana.
                </div>
              </div>
            )}
          </div>
