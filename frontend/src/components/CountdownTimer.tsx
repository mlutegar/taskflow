...
export default function CountdownTimer({ seconds, title, onComplete, onCancel }: CountdownTimerProps) {
  const [paused, setPaused] = useState<boolean>(false);
  const [displayRemaining, setDisplayRemaining] = useState<number>(seconds);

  // Referências para não recriar o interval ao cada mudança de estado
  const startedAtRef        = useRef<number | null>(null);       // Date.now() do último (re)início
  const remainingAtPauseRef = useRef<number>(seconds);    // segundos restantes no momento da pausa
