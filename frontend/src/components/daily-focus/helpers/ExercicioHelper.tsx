...
interface ExercicioHelperProps {
  state: Partial<ExercicioState>;
  onChange: (state: ExercicioState) => void;
}

export default function ExercicioHelper({ state, onChange }: ExercicioHelperProps) {
  const s: ExercicioState = { ...DEFAULT_STATE, ...state };
  const set = (rounds: number): void => onChange({ ...s, rounds });
