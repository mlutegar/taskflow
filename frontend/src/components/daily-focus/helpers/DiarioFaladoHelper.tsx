...
const TIMER_DURATIONS: number[] = [2, 3, 5];
...
type DiarioFaladoHelperProps = {
  state: Partial<DiarioFaladoState>;
  onChange: (state: DiarioFaladoState) => void;
};

export default function DiarioFaladoHelper({ state, onChange }: DiarioFaladoHelperProps) {
  const s: DiarioFaladoState = { ...DEFAULT_STATE, ...state };
  const [timerRunning, setTimerRunning] = useState<boolean>(false);
  const [timerKey, setTimerKey] = useState<number>(0);
  const [currentPrompt, setCurrentPrompt] = useState<string | null>(null);
  const [showApps, setShowApps] = useState<boolean>(false);

  const set = (field: keyof DiarioFaladoState, val: unknown): void => onChange({ ...s, [field]: val });
