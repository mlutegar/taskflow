...
export const DEFAULT_STATE: RPGState = { difficulty: "medium", xpThisSession: 0, questsDone: 0 };

interface RPGHelperProps {
  state: Partial<RPGState>;
  onChange: (state: RPGState) => void;
}

export default function RPGHelper({ state, onChange }: RPGHelperProps): JSX.Element {
  const s: RPGState = { ...DEFAULT_STATE, ...state };
  const [char, setChar] = useState<CharData | null>(() => {
    try { return JSON.parse(localStorage.getItem(LS_KEY) as string); } catch { return null; }
  });
  const [creating, setCreating] = useState<boolean>(!char);
  const [nameInput, setNameInput] = useState<string>("");
  const [classInput, setClassInput] = useState<string>("Warrior");

  const saveChar = (): void => {
    if (!nameInput.trim()) return;
    const c: CharData = { name: nameInput.trim(), class: classInput, xp: 0, totalTasks: 0, totalFocus: 0 };
  ...
  const completeQuest = (): void => {
    ...
    const xpGained = Math.round((diff as typeof DIFFICULTIES[0]).xp * bonus);
    const newXp = (char?.xp || 0) + xpGained;
    const updated: CharData = { ...char as CharData, xp: newXp, totalTasks: (char?.totalTasks || 0) + 1 };
