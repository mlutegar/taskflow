...
interface LazyFalconHelperProps {
  state: Partial<LazyFalconState>;
  onChange: (state: LazyFalconState) => void;
}

export const DEFAULT_STATE: LazyFalconState = { activity: "", cycle: 1, taskInCycle: 0, saved: [] };

export default function LazyFalconHelper({ state, onChange }: LazyFalconHelperProps) {
  const s: LazyFalconState = { ...DEFAULT_STATE, ...state };
  const [noteInput, setNoteInput] = useState<string>("");
  const [showSaveNote, setShowSaveNote] = useState<boolean>(false);
