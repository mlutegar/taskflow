...
const DURATIONS: number[] = [5, 10, 15, 20];

export interface MeditarState {
  sessions: number;
  duration: number;
}

export const DEFAULT_STATE: MeditarState = { sessions: 0, duration: 10 };

interface MeditarHelperProps {
  state: Partial<MeditarState>;
  onChange: (state: MeditarState) => void;
}

export default function MeditarHelper({ state, onChange }: MeditarHelperProps) {
  const s: MeditarState = { ...DEFAULT_STATE, ...state };
  const [timerRunning, setTimerRunning] = useState<boolean>(false);
  const [timerKey, setTimerKey] = useState<number>(0);

  const set = (field: keyof MeditarState, val: number): void => onChange({ ...s, [field]: val });
