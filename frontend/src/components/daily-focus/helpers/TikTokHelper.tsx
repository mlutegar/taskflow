...
export const DEFAULT_STATE: TikTokState = { cycle: 1, videosWatched: false, taskInCycle: 0, platform: "tiktok", variant: "prog_both" };

interface TikTokHelperProps {
  state: Partial<TikTokState>;
  onChange: (state: TikTokState) => void;
}

export default function TikTokHelper({ state, onChange }: TikTokHelperProps): JSX.Element {
  const s: TikTokState = { ...DEFAULT_STATE, ...state };
