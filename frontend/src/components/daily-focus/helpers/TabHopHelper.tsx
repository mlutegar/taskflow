...
interface TabHopState {
  tabs: Tab[];
  currentIdx: number;
  cycles: number;
}

interface TabHopHelperProps {
  state: Partial<TabHopState>;
  onChange: (state: TabHopState) => void;
}

export const DEFAULT_STATE: TabHopState = { tabs: [], currentIdx: 0, cycles: 0 };

export default function TabHopHelper({ state, onChange }: TabHopHelperProps) {
  const s: TabHopState = { ...DEFAULT_STATE, ...state };
