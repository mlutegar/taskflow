...
export const DEFAULT_STATE: CantarState = {
  suggestion: null,
  done: false,
  cantadas: [],
  showManage: false,
...
interface CantarHelperProps {
  state: Partial<CantarState>;
  onChange: (state: CantarState) => void;
}

/** Tenta ler o nome do personagem RPG (único "nome de usuário" disponível no app). */
function getUserName(): string | null {
  try {
    const char = JSON.parse(localStorage.getItem("taskflow_rpg_save") as string);
...
export default function CantarHelper({ state, onChange }: CantarHelperProps) {
  const s: CantarState = { ...DEFAULT_STATE, ...state };

  const [inputVal, setInputVal] = useState<string>("");
  const [showAdd, setShowAdd] = useState<boolean>(false);
  const [savedFeedback, setSavedFeedback] = useState<boolean>(false);
  const [animate, setAnimate] = useState<boolean>(false);
  const [albumList, setAlbumList] = useState<string[]>(() => getList());
  ...
  const handleAddAlbum = (): void => {
    ...
    setInputVal("");
    setShowAdd(false);
    clearTimeout(feedbackTimer.current as ReturnType<typeof setTimeout>);
  ...
  // ── Remover álbum ──
  const handleRemoveAlbum = (name: string): void => {
    removeAlbum(name);
    const updated = getList();
    setAlbumList(updated);
