...
const DURATIONS: Record<string, number> = { "5min": 5 * 60, "15min": 15 * 60 };
...
type UmaCoisaState = typeof DEFAULT_STATE;
...
interface UmaCoisaHelperProps {
  state: Partial<UmaCoisaState>;
  onChange: (next: UmaCoisaState | ((prev: Partial<UmaCoisaState>) => UmaCoisaState)) => void;
  modeConfig?: ModeConfig;
}

export default function UmaCoisaHelper({ state, onChange, modeConfig }: UmaCoisaHelperProps) {
  const s: UmaCoisaState = { ...DEFAULT_STATE, ...state };
  const intervalRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Detecta variante pelo id do modo (uma_coisa_5min ou uma_coisa_15min)
  const variant: string = modeConfig?.id === "uma_coisa_15min" ? "15min" : "5min";
  const totalSecs: number = DURATIONS[variant];
  ...
  useEffect(() => {
    if (s.running && s.remaining > 0 && !s.done) {
      intervalRef.current = setInterval(() => {
        onChange((prev: Partial<UmaCoisaState>): UmaCoisaState => {
          const cur: UmaCoisaState = { ...DEFAULT_STATE, ...prev };
          if (cur.remaining <= 1) {
  ...
  const mins: number = Math.floor(s.remaining / 60);
  const secs: number = s.remaining % 60;
  const progress: number = (totalSecs - s.remaining) / totalSecs;

  const r: number = 38;
  const circ: number = 2 * Math.PI * r;

  const toggle = (): void => onChange({ ...s, running: !s.running });
  const reset = (): void =>
    onChange({ ...s, remaining: totalSecs, running: false, done: false, rounds: s.rounds + (s.done ? 1 : 0) });

  const handleCoisa = (e: React.ChangeEvent<HTMLInputElement>): void =>
