...
interface EsticarState {
  pauses: number;
}

interface EsticarHelperProps {
  state: Partial<EsticarState>;
  onChange: (state: EsticarState) => void;
}

export default function EsticarHelper({ state, onChange }: EsticarHelperProps) {
  const s: EsticarState = { ...DEFAULT_STATE, ...state };
