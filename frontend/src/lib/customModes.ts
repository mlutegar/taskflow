...
export function getCustomModes(): Mode[] {
  try {
...
function _persistModes(modes: Mode[]): void {
  try { localStorage.setItem(LS_MODES, JSON.stringify(modes)); } catch {}
}

function _persistDeleted(ids: Set<string>): void {
  try { localStorage.setItem(LS_DELETED, JSON.stringify([...ids])); } catch {}
}

function _syncToBackend(modes: Mode[], deletedIds: Set<string>): void {
...
export function saveCustomMode(newMode: Mode): Mode[] | undefined {
...
export function deleteCustomMode(id: string): Mode[] | undefined {
  if (!id) return;
