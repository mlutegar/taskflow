...
function addDismissed(activity) {
  const set = getDismissed();
  set.add(activity);
  storageSet(LS_KEY_DISMISSED, [...set]);
}

function removeDismissed(activity) {
  const set = getDismissed();
  set.delete(activity);
  storageSet(LS_KEY_DISMISSED, [...set]);
}
...
// ── Drag & drop order ────────────────────────────────────────────────────────
export function getPinnedOrder() {
  return storageGet(LS_KEY_ORDER, null);
}

export function setPinnedOrder(orderedList) {
...
export function applyOrder(list) {
  ...
  const orderMap = Object.fromEntries(order.map((a, i) => [a, i]));
