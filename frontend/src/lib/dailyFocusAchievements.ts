...
function saveUnlocked(list: string[]): void {
  storageSet(LS_KEY, list);
}

export function getAllWithStatus(): (Achievement & { unlocked: boolean })[] {
 ...
 */
export function tryUnlock(ids: string[]): Achievement[] {
  const unlocked = getUnlocked();
 ...
 */
export async function loadRemoteAchievements(): Promise<void> {
  try {
    const remote: string[] = await fetchAchievements();
