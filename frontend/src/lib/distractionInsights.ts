...
interface WorstHourBlock {
  id: string;
  avgIdle: number;
  block: HourBlock | undefined;
}

interface MostIdleModeEntry {
  modeId: string;
  name: string;
  avgIdle: number;
  total: number;
...
interface DistractionInsights {
  topReasons: TopReason[];
  worstHourBlock: WorstHourBlock | null;
  mostIdleModeEntry: MostIdleModeEntry | null;
  total: number;
}

export function getDistractionInsights(): DistractionInsights | null {
  ...
  const reasonIdle: Record<string, number> = {};
  ...
  const topReasons: TopReason[] = Object.entries(reasonCount)
    .map(([id, count]) => ({
      id,
  ...
  const blockIdle: Record<string, number> = {};
  ...
  const worstHourBlock: WorstHourBlock | null = Object.entries(blockIdle)
